...
| **WS crash liveness vs BJ** | **OPEN #1** — gap in `crash-rounds-ws.js` |
...
### Wave 0 — P0 money/security

| Step | # | Action |
|------|---|--------|
| 0A | **1** | Inject `liveExternal` into `makeCrashWs`; gate `start()` same as HTTP doPlay. Probe: `crash-bj-interleave-probe.js` green. |
| 0B | **2** | Add security headers middleware (helmet or manual CSP/HSTS/frame deny). |
| 0C | **7** | Move `lockReveal()` before `TokenMode.bet()` in `tokenSlots`. |
