...
///
///         V2 CHANGE (audit Critical-4): the off-chain blackjack buy-in/settle is
///         upgraded from a SINGLE global `bjLocked[player]` accumulator to a
///         PER-SESSION lock keyed by an opaque `sessionId`. Each table session
///         gets its own independent `Session{player, locked, settled}` escrow, and
///         the house signature now BINDS the sessionId (plus player, net, nonce,
///         chainid, contract). This closes the cross-session drain: a signed
///         settle for one session can only ever release that one session's
///         principal, so two concurrent games can no longer be made to pay each
///         other's locked buy-in. Everything else (deposits, on-chain games,
///         fees, the EOA-guard + pause circuit-breaker) is unchanged.
