/* ============================================================
   baccarat-net.js — thin WebSocket client for the baccarat sub-protocol.
   Byte-faithful clone of blackjack-net.js (BJNet) with exactly the three
   protocol literals changed: swallow `bac:pong`, heartbeat sends `bac:ping`,
   synthetic net-state message is `bac:net`. The `hello` frame is IDENTICAL
   to blackjack's ({ type:"hello", address, bjToken, bjSession }) so the
   server's existing identity/token-bind block needs no new parsing.

     const net = new BacNet({ wallet: myAddress });   // wallet optional
     net.on("bac:lobby:list", (m) => render(m.rooms));
     net.send({ type: "bac:lobby:subscribe" });
   ============================================================ */
(function (root) {
  "use strict";

  function BacNet(opts) {
    opts = opts || {};
  ...
  BacNet.prototype.connect = function () {
    var self = this;
    ...
    sock.onopen = function () {
      ...
      // Identify to the hub UNCONDITIONALLY. The server gates all bac:* intents (room list / join)
      ...
      // JOINING any room. Sending hello with an empty address marks the socket identified; the server
      ...
      self._emit({ type: "bac:net", state: "open" });
    };
    sock.onmessage = function (ev) {
      if (self.ws !== sock) return; // orphaned socket must not double-emit
      self._lastRx = Date.now(); // any inbound traffic (incl. bac:pong) means the socket is alive
      var m; try { m = JSON.parse(ev.data); } catch (e) { return; }
      if (m && typeof m.type === "string") { if (m.type === "bac:pong") return; self._emit(m); }
    };
    sock.onclose = function () {
      ...
      self._emit({ type: "bac:net", state: "closed" });
  ...
  BacNet.prototype.ensureConnected = function () {
    ...
    if (rs === 1) {
      ...
      // heartbeat's own 35s staleness bound, drop it NOW (onclose reconnects and bac:net open
  ...
  BacNet.prototype._startHeartbeat = function () {
    ...
    this._hb = setInterval(function () {
      ...
      try { self.ws.send(JSON.stringify({ type: "bac:ping" })); } catch (e) {}
    }, 15000);
  };
  BacNet.prototype._stopHeartbeat = function () { if (this._hb) { clearInterval(this._hb); this._hb = null; } };

  BacNet.prototype._scheduleReconnect = function () {
    var self = this;
  ...
  BacNet.prototype._raw = function (obj) { try { this.ws.send(JSON.stringify(obj)); } catch (e) { this.queue.push(obj); } };

  BacNet.prototype.send = function (obj) {
  ...
  BacNet.prototype.close = function () { this._closedByUs = true; try { this.ws.close(); } catch (e) {} };
