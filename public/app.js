...
(function () {
  ...
  const SHARE_GAME = { 8: ["Coin Flip", "🪙"], 9: ["0-100", "🎲"], 10: ["Dice #2", "🎲"], 11: ["Crash", "🚀"], 13: ["Balloon Pop", "🎈"], 14: ["Plane", "✈️"], 15: ["Gem Vault", "💎"], 16: ["Blackjack", "🂡"], 17: ["Reef Raiders", "🐠"], 18: ["Sky Swoop", "🪂"], 19: ["Fish Shooter", "🎣"] }; // #40: CH 16–19 added (were missing → generic share branding)
  ...
  function wireBetbar() {
    ...
    setInterval(() => {
      if (document.hidden) return; // L5: don't sync the bet-bar mirror while the tab is backgrounded (nothing's visible; saves 350ms wakeups)
      if (!document.body.classList.contains("has-betbar")) return;
  ...
  function toast(msg, kind) {
    ...
    toastTimer = setTimeout(() => { t.className = "toast " + (kind || ""); toastTimer = setTimeout(() => (t.className = "toast hidden"), 300); }, 3800);
  }
  ...
  function renderWallet() {
    ...
    { const f = $("bac-frame"); if (f && f.getAttribute("src") && !bacFrameMatchesWallet(f, account) && currentGame === "baccarat") ensureBaccaratReady(); } // baccarat felt: same guarded re-bind on wallet connect
    const nb = $("net-badge");
  ...
  function lockReveal() {
    revealLock = true;
    try { if (window.TokenMode) TokenMode.holdBar(); } catch (e) {} // pin the top bar to pre-bet balance
    clearTimeout(revealLockTimer);
    revealLockTimer = setTimeout(() => {
      revealLock = false;
      try { if (window.TokenMode) TokenMode.releaseBar(); } catch (e) {}
      refreshBalances();
    }, 20000); // safety
  }
  function unlockReveal() {
    ...
    revealLock = false;
    clearTimeout(revealLockTimer);
    try { if (window.TokenMode) TokenMode.releaseBar(); } catch (e) {} // release the top-bar hold at the reveal climax
    refreshBalances();
  }
  ...
  // Resolve as soon as the on-chain game-credit balance rises above `beforeWei` (a deposit landed),
  // refreshing the display each poll. THE deposit-credit fix: a mobile/injected wallet frequently stalls
  // delivering the tx receipt (tx.wait hangs) — that's the "I had to tap 'transaction submitted' or refresh
  ...
  // sign with.
  // Keep enough ETH back for a full play CYCLE of gas — a buy-in, a top-up, a settle/cash-out and a
  // withdraw are separate on-chain txs, each paid in ETH (game credits DON'T cover gas). At 0.01 a
  // single buy-in during a Sepolia gas spike could exceed the reserve and strand the player unable to
  // pay gas ("deposits work but buy-ins don't"). 0.03 covers a whole cycle even at elevated fees.
  function depositReserveWei() { try { return E.parseEther("0.03"); } catch { return 0n; } }
  ...
  async function _refreshRoomsOnce() {
    try {
      ...
      list.innerHTML = "";
      if (!rooms.length) {
        list.innerHTML = '<li class="empty">No open rooms right now.</li>'; // U2: room creation is retired/hidden — don't prompt "Create one!"
        return;
      }
  ...
  // Clicking "Flip" plays a TOKEN flip (server-paced, no wallet popup) when a session is live, or a demo flip
  // in play-money mode. OWNER RULE: every game is TOKEN-ONLY — there is NO per-flip wallet confirmation. A
  // connected wallet that isn't in a token session must buy in first; we NEVER open the legacy on-chain
  // "Flip vs House" wallet-confirm modal for a real wallet anymore (it required `account`, so gating on
  // `account` here retires that path entirely — the code below is now an unreachable fallback).
  async function playHouse() {
    if (window.TokenMode && TokenMode.active()) return tokenFlip();
    if (demoOn) return demoFlip();
    if (account) { // connected but no token session → send them to the token buy-in, never a per-flip popup
      toast("Buy in with tokens above to play — no per-flip wallet popups 🪙", "err");
      try { const tm = document.getElementById("token-mount"); if (tm) tm.scrollIntoView({ behavior: "smooth", block: "center" }); } catch (e) {}
      return;
    }
    if (!ready()) return;
  ...
  async function tokenFlip() {
    ...
    try {
      ...
      unlockReveal(); // v6 #11: ALWAYS clear the reveal lock (+ top-bar hold) on a failed bet — gating it on the
      if (TV._seq === seq) { try { TV.idle("Token bet failed — try again"); } catch (er) {} } // seq only left it stuck for 20s when the player tuned away mid-bet
  ...
  function diceReadouts() {
    ...
    // UX: the drag handle shows the WIN CHANCE (matches the "Win Chance" readout), not the raw target — in Roll Over
    // mode the target and win chance differ, so showing the target here read as the wrong odds. Display-only.
    $("ob-flag").textContent = chance.toFixed(0) + "%";
    ...
    if (stakeWei > ((window.TokenMode && TokenMode.active()) ? usdToWei(spendableUsd()) : gameWei)) hint = "Not enough in-game balance — deposit first 👇";
  ...
  function twoDiceReadouts() {
    ...
    // UX: mirror the live bet on the big TV label as you drag/toggle (was stuck on "PLACE YOUR BET BELOW" until the
    // roll). tv.js overrides it with the result during a roll; this just gives setup-time feedback on the main screen.
    { const tvl = $("td-tv-target"); if (tvl) tvl.textContent = (over ? "OVER " : "UNDER ") + T; }
    ...
    else if (stakeWei > ((window.TokenMode && TokenMode.active()) ? usdToWei(spendableUsd()) : gameWei)) hint = "Not enough in-game balance — deposit first 👇";
    else if (maxBet > 0n && stakeWei > maxBet) hint = "Max bet is " + usdOf(maxBet);
  ...
  function loadScriptOnce(src) {
    return new Promise((res, rej) => {
      ...
      // versioned src ("...?v=1357"), so a later ?v bump is a distinct file and still loads fresh — no stale cache.
      const sel = 'script[data-loadonce="' + src.replace(/"/g, "&quot;") + '"]';
      const existing = document.querySelector(sel);
      if (existing) {
        if (existing.dataset.loaded === "1") { res(); return; }
        // A previously FAILED node isn't reused — remove it so this call retries fresh (preserves retry-after-blip).
        if (existing.dataset.failed === "1") { try { existing.remove(); } catch (e) {} }
        else { existing.addEventListener("load", () => res(), { once: true }); existing.addEventListener("error", () => rej(new Error("failed to load " + src)), { once: true }); return; }
      }
      const s = document.createElement("script");
      s.src = src; s.async = true; s.setAttribute("data-loadonce", src);
      s.onload = () => { s.dataset.loaded = "1"; res(); };
      s.onerror = () => { s.dataset.failed = "1"; rej(new Error("failed to load " + src)); };
  ...
  function ensureSlotsLoaded() {
    ...
    slotsLoadPromise = loadPixiOnce()
      .then(() => loadScriptOnce("slots.js?v=1357"))
  ...
  function ensurePressureLoaded() {
    ...
    pressureLoadPromise = loadPixiOnce()
      .then(() => loadScriptOnce("pressure-engine.js?v=1357"))
      .then(() => loadScriptOnce("pressure-render.js?v=1357"))
      .then(() => loadScriptOnce("pressure-ui.js?v=1357"))
      // optional 3D red balloon (Three.js) — falls back to the 2D balloon if it can't load
      .then(() => loadThreeOnce().then(() => loadScriptOnce("pressure3d.js?v=1357")).catch(() => {}))
      .then(() => true)
  ...
  function ensurePressureReady() {
    ensurePressureLoaded().then(() => {
      const g = buildPressureGame(); if (!g || currentGame !== "pressure") return; // #2: user switched away during the async load → don't activate an off-screen canvas
      g.setActive(true);
      g.setEthUsd(ethUsd);
  ...
  function ensurePlaneLoaded() {
    ...
    planeLoadPromise = loadPixiOnce()
      .then(() => loadScriptOnce("plane-engine.js?v=1357"))
      .then(() => loadScriptOnce("plane-render.js?v=1357"))
      .then(() => loadScriptOnce("plane-feed.js?v=1357"))
      .then(() => loadScriptOnce("plane-ui.js?v=1357"))
      .then(() => true)
  ...
  function ensurePlaneReady() {
    ...
    ensurePlaneLoaded().then(() => {
      const g = buildPlaneGame(); if (!g || currentGame !== "plane") return; // #2
      g.setActive(true);
      g.setEthUsd(ethUsd);
      if (window.TokenMode && TokenMode.active()) {
        g.setMode("token");
  ...
  function ensureSlots3dLoaded() {
    ...
    slots3dLoadPromise = loadThreeOnce()
      .then(() => loadScriptOnce("slots3d-engine.js?v=1357"))
      .then(() => loadScriptOnce("slots3d.js?v=1357"))
      .then(() => true)
  ...
  function buildSlots3d() {
    ...
    slots3dGame = new window.Slots3D({
      mount, width: 800, height: 600, ethUsd: ethUsd, initialBalance: (window.TokenMode && TokenMode.active()) ? TokenMode.tokens() : (account ? 0 : demoUsd),
      onBalance: (b) => {
        if (window.TokenMode && TokenMode.active()) {
          // token: reflect the OPTIMISTIC per-spin debit/credit the instant slots3d changes its local balance, so
          // the stake comes off the top bar + TV overlay the moment you tap SPIN. HOLD at that value ONLY during a
          // live spin (idle re-syncs must not freeze the bar); the settle path calls TokenMode.syncBalance() which
          // releases the hold + reconciles to the authoritative balance.
          try { if (slots3dGame && (slots3dGame._spinning || slots3dGame._awaitingServer)) { if (TokenMode.holdBarAt) TokenMode.holdBarAt(b); } else if (TokenMode.releaseBar) TokenMode.releaseBar(); } catch (e) {}
          try { paintTvBalance(); } catch (e) {}
          return;
        }
  ...
  function ensureSlots3dReady() {
    ensureSlots3dLoaded().then(() => {
      const g = buildSlots3d(); if (!g || currentGame !== "slots3d") return; // #2
      g.setActive(true); g.setEthUsd(ethUsd);
      // v9 #3: don't paint the entry balance while a spin/bonus is mid-air — TokenMode.tokens() already reflects a
      // credited win, so painting it now would SPOIL the reveal before the reels land. The reels' own _settle →
      // TokenMode.syncBalance() paints the balance WITH the visual, so this can never strand a stale value.
      if (!(g._spinning || g._awaitingServer || g._bonus)) g.setBalance((window.TokenMode && TokenMode.active()) ? TokenMode.tokens() : (account ? 0 : demoUsd));
      g.setEnabled(true);
  ...
  function ensureFishLoaded() {
    ...
    fishLoadPromise = loadPixiOnce()
      .then(() => loadScriptOnce("fishtable-engine.js?v=1357"))
      .then(() => loadScriptOnce("fishtable.js?v=1357"))
      .then(() => true)
  ...
  function buildFish() {
    ...
    fishGame = new window.FishTable({
      mount, width: 900, height: 600, ethUsd: ethUsd, initialBalance: (window.TokenMode && TokenMode.active()) ? TokenMode.tokens() : (account ? 0 : demoUsd),
      onBalance: (b) => { if (window.TokenMode && TokenMode.active()) return; demoUsd = Math.round(b * 100) / 100; demoSave(); demoPaint(); }, // demo only (token mode: balance owned by TokenMode)
  ...
  function ensureFishReady() {
    ...
    const boot = () => ensureFishLoaded().then(() => {
      const g = buildFish(); if (!g || currentGame !== "fish") return; // #2: prevent off-screen Reef auto-fire token bets
      g.setActive(true); g.setEthUsd(ethUsd);
      if (window.TokenMode && TokenMode.active()) g.setBalance(TokenMode.tokens()); else g.setBalance(account ? 0 : demoUsd); // v5 #4: a CONNECTED wallet shows 0 (never a stale demo balance) until buy-in — channel re-entry self-heals the 9k leak
  ...
  function ensureSwoopLoaded() {
    ...
    swoopLoadPromise = loadPlayCanvasOnce()
      .then(() => loadScriptOnce("swoop3d.js?v=1357"))
      .then(() => true)
  ...
  function buildSwoop() {
    ...
    swoopGame = new window.SwoopGame({
      ...
      onBalance: (b) => { if (window.TokenMode && TokenMode.active()) return; demoUsd = Math.round(b * 100) / 100; demoSave(); demoPaint(); }, // demo play-money (v13 #36: token mode owns the balance — never clobber a token session if swoop is re-enabled)
      ...
      onTick: (m) => { if (mult) mult.textContent = m.toFixed(2) + "x"; if (go && swoopGame && swoopGame._state === "climbing") go.textContent = "CASH OUT  " + usd(swoopGame.unitBet * m); }, // UX: show the live bankable amount (bet × mult) so the player doesn't do mental math (parity with Crash/Plane)
      onState: (s) => {
        ...
        else if (s.state === "climbing") { if (go) { go.textContent = "CASH OUT  " + usd(swoopGame.unitBet * (s.mult || 1)); go.className = "swoop-go cash"; } }
  ...
  function ensureSwoopReady() {
    ...
    const boot = () => ensureSwoopLoaded().then(() => {
      const g = buildSwoop(); if (!g || currentGame !== "swoop") return; // #2
      g.setActive(true); g.setEthUsd(ethUsd);
  ...
  function ensureFishShooterLoaded() {
    ...
    fishshooterLoadPromise = loadPixiOnce()
      .then(() => loadScriptOnce("fishshooter-engine.js?v=1357")) // OWN engine (decoupled from Reef's fishtable-engine.js)
      .then(() => loadScriptOnce("fishshooter.js?v=1357"))
      .then(() => true)
  ...
  function ensureFishShooterReady() {
    ...
    const boot = () => ensureFishShooterLoaded().then(() => {
      const g = buildFishShooter(); if (!g || currentGame !== "fishshooter") return; // #2
      g.setActive(true); g.setEthUsd(ethUsd);
      if (window.TokenMode && TokenMode.active()) g.setBalance(TokenMode.tokens()); else g.setBalance(account ? 0 : demoUsd); // v5 #4: a CONNECTED wallet shows 0 (never a stale demo balance) until buy-in — channel re-entry self-heals the 9k leak
      g.setEnabled(true);
  ...
  function ensureFishShooter2Loaded() {
    ...
    fishshooter2LoadPromise = loadPixiOnce()
      .then(() => loadScriptOnce("fishshooter2-engine.js?v=1357"))
      .then(() => loadScriptOnce("fishshooter2.js?v=1357"))
      .then(() => true)
  ...
  function loadCoinFlip3dOnce() {
    ...
    coinFlip3dLoadPromise = loadThreeOnce()
      .then(() => loadScriptOnce("coinflip3d.js?v=1357"))
      .then(() => true)
  ...
  function ensureCoinFlip3dReady() {
    loadCoinFlip3dOnce().then(() => {
      const g = buildCoinFlip3d(); if (!g || currentGame !== "flip") return; // #2
      g.setActive(true);
  ...
  function loadRail3dOnce() {
    ...
    rail3dLoadPromise = loadThreeOnce().then(() => loadScriptOnce("dice3d.js?v=1357")).then(() => true).catch((e) => { rail3dLoadPromise = null; throw e; });
  ...
  function loadDice2_3dOnce() {
    ...
    d2_3dLoadPromise = loadThreeOnce().then(() => loadScriptOnce("dice2-3d.js?v=1357")).then(() => true).catch((e) => { d2_3dLoadPromise = null; throw e; });
  ...
  // a fresh session; a buy-in/top-up/cash-out re-anchors the base so a deposit/withdrawal isn't read as P&L.
  // #97: on the Blackjack channel the felt runs in its own iframe with its OWN chips balance, so the tracker
  // follows THAT (bjLastBal) instead of demo/token credits. Blackjack is its own session "mode", so entering/
  // leaving the channel re-anchors (a mode change) — switching channels never misreads as a huge win/loss.
  function sesMode() { return currentGame === "blackjack" ? "bj" : (currentGame === "baccarat" ? "bac" : ((window.TokenMode && TokenMode.active()) ? "token" : "demo")); }
  function activeBal() {
    try {
      if (currentGame === "blackjack") return bjLastBal != null ? Math.round(bjLastBal * 100) / 100 : 0;
      if (currentGame === "baccarat") return bacLastBal != null ? Math.round(bacLastBal * 100) / 100 : 0;
      return (window.TokenMode && TokenMode.active()) ? (Math.round((TokenMode.tokens() || 0) * 100) / 100) : demoUsd;
    } catch (e) { return demoUsd; }
  }
  function sessionReset() {
    const m = sesMode();
    // On a FELT channel (bj/bac) before the iframe has reported its chips balance, activeBal()
    // falls back to 0 — anchoring there makes the whole balance read as "won" forever. Defer:
    // paintSession's `sessionBase == null` branch anchors the moment a real balance lands.
    const feltUnknown = (m === "bj" && bjLastBal == null) || (m === "bac" && bacLastBal == null);
    const b = feltUnknown ? null : activeBal();
    sessionBase = b; _sesBal = b; sesWagered = 0; sesWon = 0; _sesMode = m; paintSession();
  }
  function recordSession(wager, returned) { // demo table games report EXACT gross wagered + returned
    sesWagered = Math.round((sesWagered + Math.max(0, +wager || 0)) * 100) / 100;
    sesWon = Math.round((sesWon + Math.max(0, +returned || 0)) * 100) / 100;
    _sesBal = activeBal(); paintSession(); // this settle already moved the balance → mark it counted so the watcher skips it
  }
  ...
  function paintSession(reanchor) {
    ...
    const bal = activeBal();
    if (_sesMode !== mode) { _sesMode = mode; sessionBase = bal; _sesBal = bal; sesWagered = 0; sesWon = 0; } // demo↔token↔blackjack → fresh session (no cross-mode delta)
    else if (sessionBase == null) { sessionBase = bal; _sesBal = bal; }
    else if (_sesBal != null && bal !== _sesBal) {
      const d = Math.round((bal - _sesBal) * 100) / 100;
      if (reanchor) sessionBase = Math.round((sessionBase + d) * 100) / 100; // deposit/withdrawal → NET unchanged
      else if (d > 0) sesWon = Math.round((sesWon + d) * 100) / 100;
      else sesWagered = Math.round((sesWagered - d) * 100) / 100;
      _sesBal = bal;
    }
    const w = $("ds-wagered"), wn = $("ds-won"), nt = $("ds-net");
    if (w) w.textContent = usd(sesWagered);
    if (wn) wn.textContent = usd(sesWon);
    if (nt) { const net = Math.round((bal - sessionBase) * 100) / 100; nt.textContent = (net >= 0 ? "+" : "−") + usd(Math.abs(net)); nt.className = "ds-net " + (net > 0 ? "up" : net < 0 ? "down" : ""); }
  }
  // Bottom-right TV balance readout, on EVERY channel (Fish Shooter parity — same spot, same spendable figure via
  // tokenStateBalanceUsd). HIDDEN on channels that draw their OWN in-canvas balance (Reef "fish", "fishshooter") or
  // run in an iframe ("blackjack"). Display-only; never mutates money. Called on channel switch + every balance
  ...
  function paintTvBalance() {
    ...
    if (currentGame === "fish" || currentGame === "fishshooter" || currentGame === "fishshooter2" || currentGame === "blackjack" || currentGame === "baccarat") { el.classList.add("hidden"); return; }
    ...
    // you could bet ("$50 but can't play"). The "Buy in" bar + prompt guide the buy-in.
    var bal = 0, show = false;
    if (window.TokenMode && TokenMode.active()) { show = true; try { bal = TokenMode.displayTokens ? TokenMode.displayTokens() : TokenMode.tokens(); } catch (e) { bal = 0; } }
    else if (typeof demoOn !== "undefined" && demoOn) { show = true; bal = demoUsd; }
    if (!show) { el.classList.add("hidden"); return; }
    var v = document.getElementById("tv-balance-val");
    if (v) v.textContent = "$" + (Math.round((+bal || 0) * 100) / 100).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    el.classList.remove("hidden");
  }
  // Token bets in canvas games (fish/slots3d/…) move TokenMode.tokens() without always firing onChange, so
  // poll lightly to keep the tracker live; the delta-watcher dedupes via _sesBal so this never double-counts.
  setInterval(function () { if (document.hidden) return; try { if ((window.TokenMode && TokenMode.active()) || currentGame === "blackjack" || currentGame === "baccarat") paintSession(); } catch (e) {} try { paintTvBalance(); } catch (e) {} }, 1500); // L5: skip the session/TV-balance repaint while the tab is hidden
  ...
  function demoSyncBalance() {
    ...
    try { maxBet = usdToWei(DEMO_BET_MAX_USD); } catch {} // demo per-game bet ceiling ($1,000); token mode uses the on-chain maxBet
    ...
    // v5 #4: every ETH/USD refresh SELF-HEALS a connected wallet's canvas balance to (token-or-0), never a
    // stale demoUsd — so a demo balance can't survive a connect even if some other path missed it.
    if (planeGame) try { planeGame.setEthUsd(ethUsd); if (window.TokenMode && TokenMode.active()) planeGame.setBalance(TokenMode.tokens()); else planeGame.setBalance(account ? 0 : demoUsd); } catch (e) {}
    if (slots3dGame) try { slots3dGame.setEthUsd(ethUsd); if (window.TokenMode && TokenMode.active()) slots3dGame.setBalance(TokenMode.tokens()); else slots3dGame.setBalance(account ? 0 : demoUsd); } catch (e) {}
    if (fishGame) try { fishGame.setEthUsd(ethUsd); if (window.TokenMode && TokenMode.active()) fishGame.setBalance(TokenMode.tokens()); else fishGame.setBalance(account ? 0 : demoUsd); } catch (e) {}
    if (swoopGame) try { swoopGame.setEthUsd(ethUsd); if (demoOn) swoopGame.setBalance(demoUsd); } catch (e) {}
    if (fishshooterGame) try { fishshooterGame.setEthUsd(ethUsd); if (window.TokenMode && TokenMode.active()) fishshooterGame.setBalance(TokenMode.tokens()); else fishshooterGame.setBalance(account ? 0 : demoUsd); } catch (e) {}
    if (fishshooter2Game) try { fishshooter2Game.setEthUsd(ethUsd); if (window.TokenMode && TokenMode.active()) fishshooter2Game.setBalance(TokenMode.tokens()); else fishshooter2Game.setBalance(account ? 0 : demoUsd); } catch (e) {} // CH20 was omitted → an ethUsd/demo refresh didn't self-heal its balance (sibling of the fishshooterGame line above)
    // Keep the under-TV session tracker live. v11 #6: demoSyncBalance() takes NO `force` param, so the old
    // paintSession(force) threw a (swallowed) ReferenceError → the tracker silently never repainted here. A plain
    // repaint is correct in this demo-sync context (no deposit/withdrawal re-anchor intent to pass through).
    try { paintSession(); } catch (e) {}
    try { paintTvBalance(); } catch (e) {}
    try {
      setupSliders();
      ...
      else if (currentGame === "slots") slotsReadouts(); // v12 #4: slots affordability is now token-aware too — re-run it
    } catch (e) {}
  }
  ...
  function syncTokenGameBalances(force) {
    ...
    [slots3dGame, fishGame, fishshooterGame, fishshooter2Game].forEach(function (g) { // CH20 (fishshooter2) was OMITTED → its real-money HUD went stale on every token event (buy-in/top-up/cash-out/resume) until the next shot; the onFishChan/inBonus guards at 3532/3542 already anticipate it
      if (!g) return;
      try { g.setEthUsd(ethUsd); } catch (e) {}
      // BUGFIX: use pbBal (token tokens / demo / ELSE 0), NOT the gameWei-tainted `bal`. A CONNECTED wallet
    ...
    // it immediately so the bet controls appear on the SAME tap (one step: Buy in → playing). bjFrameMatchesWallet
    ...
    // Baccarat felt: identical buy-in re-bind (a hash change alone won't reload an iframe).
    if (currentGame === "baccarat" && account) {
      try { const f = $("bac-frame"); if (f && f.getAttribute("src") && !bacFrameMatchesWallet(f, account) && Date.now() - bacHealAt > 1500) { bacHealAt = Date.now(); ensureBaccaratReady(); } } catch (e) {}
    }
  }
  ...
  try { window.addEventListener("ctf:resume-done", function () { try { checkStrandedLock(); } catch (e) {} try { if (currentGame === "blackjack" && account) ensureBlackjackReady(); } catch (e) {} try { if (currentGame === "baccarat" && account) ensureBaccaratReady(); } catch (e) {} }); } catch (e) {} // resume finished → (re)bind the felt to the resumed session (it was deferred while resumePending)
  function enterDemo() {
  ...
  function exitDemo() {
    if (!demoOn) return;
    demoOn = false;
    // v14 #6/#7: demo mode seeds gameWei from demoUsd (for the demo Max sliders). If the FIRST connected
    // refreshBalances silently fails, that demo figure survives as "game credits" (the $5k-demo-looking
    // balance / wrong buy-in gate). Reset it here — the next refreshBalances repaints the real on-chain value.
    gameWei = 0n;
  ...
  function demoFlip() {
    ...
    recordGameResult("flip", "demo", won, v, won ? 0.94 * v : -v);
    demoFlipBusy = true;
    const fb = $("play-house-btn"); if (fb) fb.disabled = true;
    const release = () => { demoFlipBusy = false; if (fb) fb.disabled = false; };
    lockReveal();
  ...
  function demoDice() {
    ...
    recordGameResult("dice", "demo", won, v, won ? profitUsd : -v);
    lockReveal();
  ...
  function demoTwoDice() {
    ...
    demoUsd += (won ? profitUsd : -v); demoSave();
    recordSession(v, won ? v + profitUsd : 0);
    recordGameResult("twodice", "demo", won, v, won ? profitUsd : -v);
    lockReveal();
  ...
  function demoCrash() {
    ...
    demoUsd += (won ? profitUsd : -v); demoSave();
    recordSession(v, won ? v + profitUsd : 0);
    recordGameResult("crash", "demo", won, v, won ? profitUsd : -v);
    lockReveal();
  ...
  function demoSlots() {
    ...
    ensureSlotsLoaded().then(() => {
      ...
      recordGameResult("slots", "demo", won, v, (won ? winUsd : 0) - v);
      lockReveal();
  ...
  // M4: the payout ACTUALLY credited to the session (server ledger delta after the max-win cap).
  // Falls back to the raw payoutUnits for older server responses without the field.
  function tokenBanked(r) { return typeof r.bankedUnits === "number" ? r.bankedUnits : (r.payoutUnits || 0); }
  async function tokenDice() {
    ...
    const profitUsd = won ? Math.max(0, (tokenBanked(r)) - v) : 0; // M4: banner the BANKED payout (max-win cap), never raw game math
    recordGameResult("dice", "token", won, v, won ? profitUsd : -v);
    TV.revealDice({ roll: roll / 100, target: T / 100, mode: over ? "over" : "under", youWon: won, mult, amountUsd: won ? profitUsd : v, tier: demoTier(profitUsd) });
    if (r.capped) toast("Win clamped to this session's max-win cap — the capped amount is banked", "ok");
  ...
  async function tokenTwoDice() {
    ...
    const profitUsd = won ? Math.max(0, (tokenBanked(r)) - v) : 0; // M4: banked, not raw
    recordGameResult("twodice", "token", won, v, won ? profitUsd : -v);
    TV.revealTwoDice({ d1, d2, target: T, mode: over ? "over" : "under", youWon: won, mult, amountUsd: won ? profitUsd : v, tier: demoTier(profitUsd) });
    if (r.capped) toast("Win clamped to this session's max-win cap — the capped amount is banked", "ok");
  ...
  async function tokenCrash() {
    ...
    const profitUsd = won ? Math.max(0, (tokenBanked(r)) - v) : 0; // M4: banked, not raw
    recordGameResult("crash", "token", won, v, won ? profitUsd : -v);
    TV.revealCrash({ crashX, targetX, won, amountUsd: won ? profitUsd : v, mult: targetX, tier: demoTier(profitUsd) });
    if (r.capped) toast("Win clamped to this session's max-win cap — the capped amount is banked", "ok");
  ...
  async function tokenSlots() {
    ...
    if (TV._seq !== seq) { unlockReveal(); return; } // v7 #13: tuned away mid round-trip → don't hijack the TV
    const won = !!r.win, grid = (r.outcome && r.outcome.grid) || [], winUsd = won ? tokenBanked(r) : 0; // M4: banked, not raw
    recordGameResult("slots", "token", won, v, winUsd - v);
    TV.revealSlots({ grid, winUsd, betUsd: v, won });
    if (r.capped) toast("Win clamped to this session's max-win cap — the capped amount is banked", "ok");
  ...
  function paintGameTabs(game) {
    ...
    document.body.classList.toggle("game-baccarat", game === "baccarat");
  ...
  function switchGame(game) {
    ...
    currentGame = game;
    paintGameTabs(game);
    try { paintTvBalance(); } catch (e) {} // update the bottom-right TV balance readout instantly on channel change (show/hide + value)
    ...
    if (game !== "baccarat" && window.BAC_MUTE) { window.BAC_MUTE(true); bacStopCount(); bacDockSig = ""; } // same for the baccarat felt
    ...
    else if (game === "crash") { if (!window.CrashRender) loadScriptOnce("crash-render.js?v=1357").then(() => { try { if (window.TV && TV._crashIdle && currentGame === "crash") TV._crashIdle(); } catch (e) {} }).catch(() => {}); refreshDiceHouse(); crashReadouts(); ensureCrashSupport(); }
    ...
    else if (game === "baccarat") { ensureBaccaratReady(); }
  }
  ...
  function bjFrameMatchesWallet(frame, wallet) {
    ...
    try {
      ...
      if (String(u.searchParams.get("guest") || "").toLowerCase() !== String(wallet).toLowerCase()) return false;
      // A BUY-IN keeps the same wallet but opens a NEW token session; the felt must reload with the #bjsession
      // hash to bind to it (changing only an iframe's #hash does NOT reload it). So if the player is token-active
      // and the felt's hash isn't carrying the LIVE session id, treat it as a MISMATCH → the self-heal re-binds
      // it, and the bet controls appear right after the one-tap buy-in (no manual "lock credits" step).
      try {
        const si = (window.TokenMode && TokenMode.active && TokenMode.active() && TokenMode.session) ? TokenMode.session() : null;
        const liveSid = si && si.sessionId ? String(si.sessionId) : "";
        if (liveSid) {
          const feltSid = String(new URLSearchParams((src.split("#")[1] || "")).get("bjsession") || "");
          if (feltSid !== liveSid) return false;
        }
      } catch (e) {}
      return true;
  ...
  function ensureBlackjackReady() {
    ...
    // While a SAVED token session is still resuming (page reload / return from the wallet app), active() is
    // false — building the felt NOW binds it as a $0 wallet-guest with NO #bjsession ("Waiting for the next
    // hand…" with the token bar showing a live balance). Defer: resumePending ALWAYS clears (success or fail)
    // and dispatches ctf:resume-done, which re-runs this with the session available.
    ...
    if (f && !f.getAttribute("src")) {
      ...
      let src = "blackjack.html?tv=1&v=1357&r=" + (++bjFeltNonce % 8) + "&guest=" + encodeURIComponent(tableWallet); // %8: consecutive nonces still ALWAYS differ (n vs n+1 mod 8) so the iframe truly reloads, but the URL set is bounded → the SW's ?v= cache-first path can actually HIT (instant felt load from cache) instead of storing a new never-reusable copy per open
      let tokenHash = "";
  ...
  async function bjReload() {
    const f = $("bj-frame");
    if (!account) {
      if (f && f.contentWindow) try { f.contentWindow.postMessage({ type: "bj:seed", balance: Math.round(demoUsd * 100) / 100 }, location.origin); } catch (e) {} // #24 / cluster-a: seed the guest table from the site demo balance (demoUsd), not the hard-coded BJ_START — keeps the felt in sync with the rest of the site
      return;
    }
  ...
  function paintBjConnectedBalance(state) {
    ...
    // (The legacy "Lock credits" reload button is hidden for connected players — renderBjDock — so there's no
    // button text to set here anymore; buy-in / cash-out are the token bar's job.)
  }
  ...
  function bjCmd(cmd, extra) { const f = $("bj-frame"); if (f && f.contentWindow) try { f.contentWindow.postMessage(Object.assign({ type: "bj:cmd", cmd }, extra || {}), location.origin); } catch (e) {} } // #24: same-origin iframe only (the one bj postMessage the first sweep missed — nested Object.assign parens)
  ...
  function buildBjBet(ctr, s) {
    if (!(s.betMax >= 10)) {
      ...
      lbl.textContent = account ? "Tap “Buy in” in the bar above ↑ to start playing." : "Reload chips before placing a bet.";
  ...
  function buildBjActions(ctr, s) {
    ...
    ["hit", "stand", "double", "split", "surrender"].forEach((a) => {
      ...
      if (needTopUp) {
        ...
        b.className = "btn bj-act " + BJ_ACT[a][1];
        // UX: DOUBLE and SPLIT stake an ADDITIONAL bet equal to your hand bet — show that on the button so it's not
        // a surprise. (Display-only; the actual stake is unchanged and enforced server-side.)
        if ((a === "double" || a === "split") && s.handBet > 0) b.innerHTML = BJ_ACT[a][0] + ' <span class="bj-tu-sub">+$' + Math.round(s.handBet) + "</span>";
        else b.textContent = BJ_ACT[a][0];
  ...
  function renderBjDock(s) {
    ...
    try {
      const f0 = $("bj-frame");
      // v13.57: the felt is ALSO stale if it's bound to a DIFFERENT (or no) token bjsession than the live one —
    ...
    // #97: for a GUEST (no wallet), replace the "🪪 —" placeholder with a clean chips label so the walletrow reads well.
    if (!account) { const wl0 = $("bj-wallet"); if (wl0) wl0.textContent = "🃏 Table chips"; }
    // #97: feed the felt's chips balance into the site-wide THIS SESSION tracker (wagered/won/net). The tracker's
    // delta-watcher classifies each hand's balance change; a RELOAD is a deposit → re-anchor so it's not read as a win.
    if (s.balance != null) {
    ...
    if (account) {
      ...
      // The "Lock credits" / table "Cash out" buttons drive the LEGACY on-chain blackjack bridge, which is
      // disabled — a CONNECTED player buys in + cashes out from the token bar above (one tap, auto-locks). So
      // hide them for ANY connected wallet (was: only when already token-funded → a not-yet-bought-in player
      // still saw the dead "Lock credits" button). Only a DEMO guest keeps the reload (it just reseeds $1,000).
      const hideLegacy = !!account;
      if (reloadB) reloadB.style.display = hideLegacy ? "none" : "";
      if (cashoutB) cashoutB.style.display = hideLegacy ? "none" : "";
      ...
      if (status && tableUsd <= 0) {
        status.innerHTML = tokenFunded
          ...
          : (account
            ? "Tap <b>Buy in</b> in the bar above ↑ to fund this table — then place your bet."
            : "Reload chips to start playing.");
      }
    }
  ...
  // ── Baccarat channel (CH 21): the live Punto Banco felt runs in an isolated iframe
  // (blackjack pattern — its own CSS/scripts can't collide with the site). The chip tray +
  // PLAYER/TIE/BANKER zone buttons are native site elements in the dock under the TV,
  // bridged to the felt via postMessage (bac:cmd add/undo/clear/rebet — spec §5: tap-a-zone,
  // NO confirm button; chips on the felt at window close ARE the bet). Chip selection is
  ...
  function bacFrameMatchesWallet(frame, wallet) {
    if (!frame || !frame.getAttribute("src") || !wallet) return false;
    try {
      const src = frame.getAttribute("src");
      const u = new URL(src, location.href);
      if (String(u.searchParams.get("guest") || "").toLowerCase() !== String(wallet).toLowerCase()) return false;
      // A BUY-IN keeps the same wallet but opens a NEW token session; the felt must reload with the #bjsession
      // hash to bind to it (changing only an iframe's #hash does NOT reload it) — bjFrameMatchesWallet clone.
      try {
        const si = (window.TokenMode && TokenMode.active && TokenMode.active() && TokenMode.session) ? TokenMode.session() : null;
        const liveSid = si && si.sessionId ? String(si.sessionId) : "";
        if (liveSid) {
  ...
  let bacFeltNonce = 0; // bumps each felt (re)load so the iframe TRULY reloads (a #hash change alone doesn't)
  function ensureBaccaratReady() {
    if (!BACCARAT_ENABLED()) return; // kill switch — a stale saved-channel restore must not boot a disabled game
    const f = $("bac-frame");
    // NEVER tear down / reload the felt iframe while money rides the current coup (v5 #10 pattern):
    // dropping the src kills the felt's socket mid-round. Re-post the active state and defer the re-bind.
    if (f && f.getAttribute("src") && bacDockLive) { bacFramePost(true); return; }
    if (f && f.getAttribute("src") && account && !bacFrameMatchesWallet(f, account)) f.removeAttribute("src");
    // Token session changed (bought in / cashed out) since the felt loaded → reload to re-bind (safe: token
    // cash-out is refused mid-coup server-side, and bacDockLive above blocks a mid-round teardown).
    if (f && f.getAttribute("src")) {
      let wantSid = "";
      try { if (account && window.TokenMode && TokenMode.active && TokenMode.active() && TokenMode.session) { const si = TokenMode.session(); wantSid = (si && si.sessionId) || ""; } } catch (e) {}
      let curSid = "";
    ...
    // While a SAVED token session is still resuming, defer — ctf:resume-done re-runs this (bj clone).
    if (f && !f.getAttribute("src") && account && window.TokenMode && TokenMode.resumePending && TokenMode.resumePending()) return;
    if (f && !f.getAttribute("src")) {
      const tableWallet = account || bjGuestId(); // REUSE the blackjack guest identity — one guest + one demo balance spans both tables
      let src = "baccarat.html?tv=1&v=1357&r=" + (++bacFeltNonce % 8) + "&guest=" + encodeURIComponent(tableWallet); // %8 bounds the URL set so the SW ?v= cache can HIT (bj pattern)
      let tokenHash = "";
      // Real-money path: fund the table with the player's TOKEN session (chips = tokens; hello frame identical to BJ).
      if (account && window.TokenMode && TokenMode.active && TokenMode.active() && TokenMode.session) {
        try { const si = TokenMode.session(); if (si && si.sessionId) tokenHash = "#bjtoken=" + encodeURIComponent(si.sessionToken || "") + "&bjsession=" + encodeURIComponent(si.sessionId); } catch (e) {}
      }
      if (bacPendingTable) { src += "&table=" + encodeURIComponent(bacPendingTable); bacPendingTable = null; }
      f.src = src + tokenHash; // loads the felt + scripts inside the TV (no legacy-bridge fallback — token-only, spec §8)
    }
    setTimeout(() => { bacFramePost(true); }, 50);
  }
  window.BAC_MUTE = (mute) => bacFramePost(!mute); // hush the felt's audio when off-channel
  function bacShareLink() { return location.origin + location.pathname + "?game=baccarat" + (bacRoomId ? "&bactable=" + encodeURIComponent(bacRoomId) : ""); }
  function bacShareTable() {
    const link = bacShareLink();
    const done = () => toast("Table link copied — send it to a friend to sit down with you", "ok");
    try { navigator.clipboard.writeText(link).then(done, () => { window.prompt("Copy this table link:", link); }); }
    catch (e) { window.prompt("Copy this table link:", link); }
  }
  // Fullscreen (spec §5.6): the felt posts bac:fs {on}; the parent promotes #layer-baccarat to a
  // fixed full-viewport overlay + attempts native requestFullscreen, then answers bac:active {fs}
  // (the felt flips body.fs-embed and renders its own bet bar). DELIBERATELY promoted IN PLACE, not
  // the fish rr-fs reparent-to-<body>: moving an <iframe> in the DOM reloads its document, which
  // would drop the felt's socket mid-round (fish layers hold canvases, so reparenting is safe there).
  // No ancestor of the layer forms a fixed-position containing block, so the overlay fills the
  // viewport; entry/exit never touches the socket or iframe src.
  function bacSetFullscreen(on) {
    const layer = $("layer-baccarat"); if (!layer) return;
    on = !!on;
    if (on !== bacFsOn) {
      bacFsOn = on;
  ...
  // the felt now has its OWN dial (fullscreen bar) writing the same key — a storage event
  // (fires cross-context) keeps this dock's slider/readout in step, in place
  window.addEventListener("storage", (e) => {
    if (!e || e.key !== "bacChip") return;
    const v = parseInt(e.newValue, 10);
    if (!isFinite(v) || v < 10 || v === bacChip) return;
    bacChip = Math.round(v / 5) * 5;
    const sl = document.querySelector("#bac-controls .bac-amt"), lbl = document.querySelector("#bac-controls .bac-amt-val");
    if (sl) { sl.value = String(bacChip); sl.setAttribute("aria-valuetext", bacMoney(bacChip)); }
    if (lbl) lbl.textContent = bacMoney(bacChip);
  });
  ...
  let bacDockSig = "";
  let bacHealAt = 0;   // throttle for the felt self-heal (re-init a guest-stuck felt after the wallet connects)
  let bacDockLive = false; // money rides the current coup (chips placed, or deal/reveal in flight) — refuses a mid-round iframe teardown
  let bacTokenSyncAt = 0;  // debounce for refreshing the top token bar while at a token table
  let bacCountTimer = null, bacBetEndAt = 0;
  function bacStopCount() { if (bacCountTimer) { clearInterval(bacCountTimer); bacCountTimer = null; } }
  function bacPaintCount() {
    const el = $("bac-count"); if (!el) { bacStopCount(); return; }
    const s = Math.max(0, Math.ceil((bacBetEndAt - Date.now()) / 1000));
    el.textContent = "⏱ " + s + " sec"; el.classList.toggle("crit", s <= 3);
    if (bacBetEndAt - Date.now() <= 0) bacStopCount();
  }
  ...
  function buildBacControls(ctr, s) {
    ...
    try { if (localStorage.getItem("bacChip") !== String(bacChip)) persistAmt(); } catch (e) {} // persist the clamp so the felt's tap amount can never exceed this readout (review v13.57 #1)
    ...
    BAC_TRAY.forEach((cd) => {
      ...
      b.style.setProperty("--cc", cd[1]); b.textContent = "$" + v; // no "+" — it crowded the circle at small dock scales; the tap-adds semantics live in the aria-label + behavior
    ...
    ctr.appendChild(zr);
    // UNDO (LIFO) / CLEAR — server escrow refunds; they VANISH with the betting window (spec §5.3)
    if (s.canUndo || s.canClear) {
      const row = document.createElement("div"); row.className = "bac-ctlrow";
      const un = document.createElement("button"); un.type = "button"; un.className = "btn btn-ghost bac-ctl"; un.textContent = "↩ UNDO";
      if (!s.canUndo) { un.disabled = true; un.setAttribute("aria-disabled", "true"); }
      un.onclick = () => bacCmd("undo");
      const cl = document.createElement("button"); cl.type = "button"; cl.className = "btn btn-ghost bac-ctl bac-clear"; cl.textContent = "✕ CLEAR";
      if (!s.canClear) { cl.disabled = true; cl.setAttribute("aria-disabled", "true"); }
      cl.onclick = () => bacCmd("clear");
      row.append(un, cl); ctr.appendChild(row);
    }
    // REBET / REBET ×2 — only while the current bets are empty; amounts PRINTED (spec R12)
    if (s.rebet && !(s.totalStaked > 0)) {
      const row = document.createElement("div"); row.className = "bac-ctlrow";
      [["⟳ REBET " + bacMoney(s.rebet.total), 1, s.rebet.ok], ["⟳⟳ REBET ×2 " + bacMoney(s.rebet.total2), 2, s.rebet.ok2]].forEach((def) => {
        const b = document.createElement("button"); b.type = "button"; b.className = "btn btn-ghost bac-ctl bac-rebet"; b.textContent = def[0];
        if (!def[2]) { b.disabled = true; b.setAttribute("aria-disabled", "true"); }
        b.onclick = () => bacCmd("rebet", { mult: def[1] });
        row.appendChild(b);
      });
      ctr.appendChild(row);
    }
  }
  let bacStatRecorded = false; // one profile-ledger record per settled coup (edge-triggered on the settle mode)
  function renderBacDock(s) {
    const status = $("bac-status"), ctr = $("bac-controls"); if (!ctr) return;
    // FS drift healer (owner's Safari rotate bug: felt stranded in fs-embed while the overlay was off —
    // chips bar inside the TV, no Exit button): the felt reports its fs belief on every dock state; if it
    // disagrees with ours, re-post the authoritative state so both sides converge within one render.
    if (s.fs != null && !!s.fs !== bacFsOn) bacFramePost(currentGame === "baccarat");
    ...
    // SELF-HEAL (bj clone): wallet connected but the felt is guest-bound / on a stale token session →
    // re-init so it binds to the account + live session. 3s throttle; NEVER while live.
    try {
      const f0 = $("bac-frame");
      let feltStale = false;
      try {
      ...
      if (account && f0 && f0.getAttribute("src") && !bacDockLive && Date.now() - bacHealAt > 3000 && (!bacFrameMatchesWallet(f0, account) || feltStale)) {
        bacHealAt = Date.now();
        const wl = $("bac-wallet"); if (wl) wl.textContent = "🪪 " + short(account);
        f0.removeAttribute("src"); ensureBaccaratReady();
        return;
      }
    } catch (e) {}
    if (status) status.innerHTML = s.msg || "Taking a seat at a live table…";
    // walletrow: balance + your chips on the felt (spec §5.4: "💰 $940 · FELT $75")
    const balEl = $("bac-bal");
    if (balEl) {
      const tokenFunded = !!(account && window.TokenMode && TokenMode.active && TokenMode.active());
      const balTxt = s.balance != null ? (tokenFunded ? "🪙 $" + Number(s.balance).toLocaleString() + " tokens" : "💰 $" + Number(s.balance).toLocaleString()) : "";
      balEl.textContent = balTxt + (s.totalStaked > 0 ? " · FELT " + bacMoney(s.totalStaked) : "");
    }
    if (!account) { const wl0 = $("bac-wallet"); if (wl0) wl0.textContent = "🎴 Table chips"; }
    { const rb = $("bac-reload"); if (rb) { const guest = !account && !(window.TokenMode && TokenMode.active && TokenMode.active()); rb.classList.toggle("hidden", !guest); rb.disabled = !!bacDockLive; } } // demo reload: guests only, parked while a coup rides (the server refuses a mid-coup seed anyway)
    if (s.balance != null && s.mode !== "waiting" && s.mode !== "spectating") { // pre-seat dock states carry a placeholder 0 — recording it anchors the session tracker at $0 and the whole balance reads as "won"
      const nb = Math.round(Number(s.balance) * 100) / 100;
      const changed = nb !== bacLastBal;
      bacLastBal = nb;
      // DEMO unification (v12.88 pattern): while on the baccarat channel the felt's guest bank is the source
      // of truth → write it back to the site demoUsd (one demo balance across every game, incl. blackjack —
    ...
    if (account) {
      ...
      // server read via refreshTokens, debounced ≤ once/1.5s — bj clone).
      try {
        const f1 = $("bac-frame");
        if (window.TokenMode && TokenMode.active && TokenMode.active() && f1 && bacFrameMatchesWallet(f1, account) && TokenMode.refreshTokens && Date.now() - bacTokenSyncAt > 1500) {
          bacTokenSyncAt = Date.now();
          TokenMode.refreshTokens();
        }
      } catch (e) {}
    }
    // countdown: re-anchor only on >900ms drift so smoothed jitter never bounces the seconds (bj clone)
    if (s.countMsLeft != null && s.countMsLeft > 0) {
      const next = Date.now() + s.countMsLeft;
      if (Math.abs(next - bacBetEndAt) > 900) { bacBetEndAt = next; bacPaintCount(); }
      if (!bacCountTimer) bacCountTimer = setInterval(bacPaintCount, 250);
    ...
    if (s.mode === "betting") {
      ...
      if (sig !== bacDockSig || !ctr.firstChild) { bacDockSig = sig; ctr.innerHTML = ""; buildBacControls(ctr, s); }
      return;
    }
    bacDockSig = ""; ctr.innerHTML = ""; // controls VANISH outside the betting window (spec §5.3); the status line narrates the coup
  }
  window.addEventListener("message", (e) => {
    const f = $("bac-frame"); if (!f || e.source !== f.contentWindow || e.origin !== location.origin) return; // same-origin + EXACT source only (#8 pattern)
    const d = e.data; if (!d) return;
    if (d.type === "bac:ready") { bacFramePost(currentGame === "baccarat"); return; } // ack-driven re-post (v5 #27): felt just booted → ship it the active/seed state now
    if (d.type === "bac:fs") { bacSetFullscreen(!!d.on); return; } // felt's ⛶ — the parent owns the layer
    if (d.type !== "bac:dock") return;
  ...
  function initDice() {
    ...
    try {
      ...
      else if (qp.get("bactable")) saved = "baccarat";
    } catch (e) {}
    ...
    if (saved === "crash") { if (window.TV && TV._crashIdle) { try { TV._crashIdle(); } catch (e) {} } if (!window.CrashRender) loadScriptOnce("crash-render.js?v=1357").then(() => { try { if (window.TV && TV._crashIdle && currentGame === "crash") TV._crashIdle(); } catch (e) {} }).catch(() => {}); }
    ...
    if (saved === "baccarat") { document.body.classList.add("bac-channel"); { const wl = $("bac-wallet"); if (wl) wl.textContent = "🪪 " + short(account || bjGuestId()); } ensureBaccaratReady(); if (window.TV && TV._baccaratIdle) try { TV._baccaratIdle(); } catch (e) {} } // restore + show the CH 21 felt on reload
  }
  ...
  // wallet balance.
  const HARD_MAX_USD = 100; // TOKEN (real-money) per-game bet ceiling. betCapUsd() = min(read.maxBet≈$500, HARD_MAX_USD),
                            // so token bets cap at $100 client-side with no contract change (on-chain $500 limit is a no-op).
  const DEMO_BET_MAX_USD = 1000; // DEMO (play-money) per-game bet ceiling — far higher so demo play feels unlimited. Real money is untouched.
  const DEPOSIT_MAX_USD = 100000;
  function betCapUsd() {
    // Demo lifts the ceiling to DEMO_BET_MAX_USD; token keeps the real $100 cap (min with the on-chain maxBet).
    const ceil = (window.TokenMode && TokenMode.active()) ? HARD_MAX_USD : DEMO_BET_MAX_USD;
    const m = (maxBet && maxBet > 0n) ? Math.floor(weiToUsd(maxBet)) : ceil;
    return Math.max(10, Math.min(ceil, m));
  }
  // The static-max core bet sliders (dice/two-dice/crash/table/PvP) sit at $500 in the HTML. In DEMO mode lift
  // them to the demo ceiling so play-money bets can reach $1,000; in TOKEN mode restore each game's real $500
  // slider. Shooters (fish/reef) own their own bet UI and are NEVER touched here (owner rule). Self-heals on
  // mode switch because setupSliders() re-runs on connect/disconnect.
  const CORE_TOKEN_SLIDER_MAX = { "dice-stake": 500, "td-stake": 500, "crash-stake": 500 };
  function applyBetCaps() {
    const demo = !(window.TokenMode && TokenMode.active());
    for (const id in CORE_TOKEN_SLIDER_MAX) {
      const s = $(id); if (!s) continue;
      s.max = String(demo ? DEMO_BET_MAX_USD : CORE_TOKEN_SLIDER_MAX[id]);
      if (+s.value > +s.max) { s.value = s.max; try { setSliderUsd(id); } catch (e) {} }
    }
  }
  function setupSliders() {
    ...
    applyBetCaps(); // demo lifts the dice/two-dice/crash sliders to $1,000; token restores $500
  }
  ...
  async function renderProfileStats() {
    ...
    // The local results ledger fills what the chain can't see (demo + token rounds, every game).
    // Address-keyed, so another player's ledger only exists here if they played in this browser.
    const local = (window.Profile && Profile.loadResults) ? Profile.loadResults(addr) : [];
    const s = Profile.computeStats(addr, { rooms, dice, twoDice, local, usdToWei: (u) => usdToWei(u) }, eq);
    ...
    if (hist) {
      ...
      // per-game W-L breakdown (games actually played, most-played first)
      let per = "";
      try {
  ...
  function connectWS() {
    ...
    try {
      ...
      // v13.18: scope both handlers to the CAPTURED sock. The v13.09 visibilitychange kick can open socket B while
      // socket A is still CLOSING; A's late onclose must NOT run stopWsHeartbeat()/reconnect against the global
      // (that would clear B's fresh heartbeat → half-open detection dead → a crash cash-out can silently drop).
      ws.onclose = () => { if (ws !== sock) return; stopWsHeartbeat(); setTimeout(connectWS, Math.min(600 * Math.pow(1.6, Math.min(wsTries++, 10)), 8000)); }; // M2: reconnect forever with capped exponential backoff (0.6s→8s); wsTries resets on open.
      ws.onerror = () => { try { sock.close(); } catch {} }; // close the SPECIFIC socket that errored, not whatever is currently global
  ...
  function wireUI() {
    ...
    // ── Resizable mobile betting sheet (owner 2026-07-02): drag the grip to shrink the whole dock so
    // more of the TV shows — everything inside zooms together. Shared per-device preference; double-tap resets.
    { const wireDockGrip = (gripId, dockSel) => {
        const grip = $(gripId), dock = document.querySelector(dockSel);
        if (!grip || !dock) return;
        const KEY = "feltDockScale";
        let scale = 1;
        try { const s = parseFloat(localStorage.getItem(KEY)); if (isFinite(s) && s >= 0.55 && s <= 1) scale = s; } catch (e) {}
        const apply = () => dock.style.setProperty("--dock-scale", String(scale));
        apply();
        ...
        grip.addEventListener("pointermove", (e) => {
          if (!dragging) return;
          const dy = e.clientY - startY; // drag DOWN = shrink the sheet
          scale = Math.max(0.55, Math.min(1, Math.round((startScale - dy / 260) * 100) / 100));
          apply();
        });
    ...
    {
      // OWNER: the intro promo reel is OFF for now (per request — re-enable in the future). Flip PROMO_ENABLED
      // to true to bring it back; no other change needed. With it off, playPromo is a no-op → _promoPlaying stays
      // false → every game reveals/idles normally (music + game reveal do NOT depend on the promo ending).
      const PROMO_ENABLED = false;
      const playPromo = () => { if (!PROMO_ENABLED) return; if (window.TV && TV.playPromo) { try { TV.playPromo(); } catch (e) {} } };
      let build = "", seen = null;
    ...
    setInterval(() => {
      ...
      // A buy-in lock tx that mines AFTER the client gave up waiting (stalled/late) otherwise leaves funds
      // in bjLocked with NO Recover button until a page reload — the "my money just disappeared" report.
      // checkStrandedLock is self-guarding (bails on active/resuming/in-flight buy-in) so this can't clobber
      // a live session; setStranded only re-renders on change. One cheap eth_call per 12s tick.
      try { checkStrandedLock(); } catch (e) {}
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    // Coming back from the wallet app (mobile foregrounds MetaMask during a confirm, freezing our timers): re-read
    // the authoritative on-chain game CREDITS and any stranded lock now, so a deposit that credited or a buy-in
    // lock that mined while we were away shows immediately — not after the next 12s tick or a manual reload.
    // (refreshBalances is a plain on-chain read; checkStrandedLock self-guards against an active/mid-flight session.)
    try { if (account && read && chainOK) { refreshBalances(); checkStrandedLock(); } } catch (e) {}
    // Blackjack felt stuck after app-switching ("Waiting for the next hand…" as a $0 wallet-guest while the
    // token bar shows a live session): iOS can evict/rebuild the iframe while backgrounded, and the periodic
    // self-heal only runs on token onChange — which nothing fires here. Re-run the GUARDED re-bind directly:
    // it no-ops unless the felt is missing/stale (bjDockLive blocks any mid-hand teardown).
    try { if (currentGame === "blackjack" && account) ensureBlackjackReady(); } catch (e) {}
    // Baccarat felt: identical guarded re-bind after app-switching (no-ops unless missing/stale; never mid-coup).
    try { if (currentGame === "baccarat" && account) ensureBaccaratReady(); } catch (e) {}
  });
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    { const goExtras = () => { if (!window.WinScenes) loadScriptOnce("scenes.js?v=1357").catch(() => {}); if (!window.Chiptune) loadScriptOnce("chiptune.js?v=1357").then(() => { try { syncSoundBtn(); } catch (e) {} }).catch(() => {}); };
