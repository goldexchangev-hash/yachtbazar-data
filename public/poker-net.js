/* ============================================================
   poker-net.js — thin WebSocket client for the poker (pk:) sub-protocol.
   Byte-faithful clone of baccarat-net.js (BacNet), with exactly the three
   protocol literals changed: swallow `pk:pong`, heartbeat sends `pk:ping`,
   synthetic net-state message is `pk:net`. The `hello` frame is IDENTICAL to
   blackjack/baccarat ({ type:"hello", address, bjToken, bjSession }) so the
   server's existing identity/token-bind block needs no new parsing — ONE token
   session funds blackjack, baccarat AND poker (server OR's the live guards).

   Poker is TURN-BASED with HIDDEN hole cards, so the server pushes a PER-VIEWER
   masked `pk:state` (snapshotFor(viewer)) — this client is a THIN RENDERER of
   those snapshots and holds NO game logic, NO deck, NO opponents' holes. The
   heartbeat/resume (below) re-pulls the snapshot after a backgrounded-mobile
   socket wakes so a frozen felt can never miss its turn.

     const net = new PokerNet({ wallet: myAddress });   // wallet optional
     net.on("pk:state", (m) => render(m));
     net.on("pk:lobby:list", (m) => renderLobby(m.rooms));
     net.send({ type: "pk:lobby:subscribe" });
   ============================================================ */
(function (root) {
  "use strict";

  function PokerNet(opts) {
    opts = opts || {};
  ...
  PokerNet.prototype.connect = function () {
    var self = this;
    ...
    sock.onopen = function () {
      ...
      // Identify to the hub UNCONDITIONALLY. The server gates all pk:* intents (lobby / join)
      // behind a seen `hello`, so a guest — or a player whose wallet/token hasn't loaded yet when the
      // felt's socket opens (common on a phone) — would otherwise be silently blocked from SEEING or
      // JOINING any table. Sending hello with an empty address marks the socket identified; the server
      // binds the token session when bjToken/bjSession are present, else treats it as a guest.
      ...
      self._emit({ type: "pk:net", state: "open" });
    };
    sock.onmessage = function (ev) {
      if (self.ws !== sock) return; // orphaned socket must not double-emit
      self._lastRx = Date.now(); // any inbound traffic (incl. pk:pong) means the socket is alive
      var m; try { m = JSON.parse(ev.data); } catch (e) { return; }
      if (m && typeof m.type === "string") { if (m.type === "pk:pong") return; self._emit(m); }
    };
    sock.onclose = function () {
      ...
      self._emit({ type: "pk:net", state: "closed" });
  ...
  PokerNet.prototype.ensureConnected = function () {
    ...
    if (rs === 1) {
      ...
      // heartbeat's own 35s staleness bound, drop it NOW (onclose reconnects and pk:net open
      // re-pulls the snapshot) instead of waiting up to 15s for the next heartbeat tick.
  ...
  // and re-pull a fresh masked snapshot, instead of sitting on a frozen felt forever (and missing a turn).
  PokerNet.prototype._startHeartbeat = function () {
    ...
    this._hb = setInterval(function () {
      ...
      try { self.ws.send(JSON.stringify({ type: "pk:ping" })); } catch (e) {}
    }, 15000);
  };
  PokerNet.prototype._stopHeartbeat = function () { if (this._hb) { clearInterval(this._hb); this._hb = null; } };

  PokerNet.prototype._scheduleReconnect = function () {
    var self = this;
  ...
  PokerNet.prototype._raw = function (obj) { try { this.ws.send(JSON.stringify(obj)); } catch (e) { this.queue.push(obj); } };

  PokerNet.prototype.send = function (obj) {
  ...
  PokerNet.prototype.close = function () { this._closedByUs = true; try { this.ws.close(); } catch (e) {} };

  root.PokerNet = PokerNet;
