...
(function (root) {
  ...
  Slots3D.prototype._showWinFx = function (res, bet) {
    ...
    // A PAID base spin can pay back LESS than the stake (a single low line only stakes bet/20), so its PROFIT would
    // be 0 and "WIN $0.00" read as broken. Owner's call: show it as a WIN of the reels' ACTUAL payout — the GROSS
    // amount — so a hit reads as a real (if small) win. A win ABOVE the stake keeps the site-wide PROFIT display
    // (winnings above your bet). Money is unchanged either way: _settle credits the gross winUsd, so this is
    // DISPLAY-ONLY (the balance / net is always correct — a sub-stake hit is still a net loss on the balance).
    const subStake = !this._bonus && res.winUsd <= bet; // reels paid back ≤ the stake → show the GROSS as the win
    ...
    // Count-up: a win ABOVE the stake shows PROFIT (winnings above your bet); a FREE bonus spin's whole win is
    // profit; a SUB-STAKE hit shows the GROSS amount the reels paid (so it's a real win, never $0.00).
    const _winShow = this._bonus ? res.winUsd : (subStake ? res.winUsd : Math.max(0, res.winUsd - bet));
